...
import { Calendar, Clock, Verified } from "lucide-react"

export default function JobCard({ job }: { job: any }) {
  const media = job.media?.length ? job.media[0] : null
  ...
  const description = getFirstParagraph(job.content)
          ...
          <div className="flex items-center justify-between pt-4 border-t">
            <div className="flex items-center gap-3">
              <Avatar className="h-8 w-8">
                ...
                <AvatarFallback className="text-xs">{job.user?.name?.charAt(0).toUpperCase() || "U"}</AvatarFallback>
              </Avatar>
              <div className="flex flex-col">
                <span className="text-sm font-medium leading-none">{job.user?.name}</span>
                <span className="text-xs text-muted-foreground mt-1">{job.user?.email}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            ...
            {job.content && (
              <div className="flex items-center gap-1">
                ...
                <span>{calculateReadingTime(JSON.stringify(job.content))}</span>
              </div>
            )}
          </div>
        </CardContent>
